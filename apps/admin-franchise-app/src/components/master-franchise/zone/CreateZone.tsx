...
const CreateZone = () => {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			const res = await fetch("/api/master-franchise/zones/create", {
				method: "POST",
			...
			if (result.message == "success") {
				...
				toast.success(`Country "${zone.name}" created successfully!`);
				setName("");
				setCode("");
			}
		} catch (err) {
		...
		<div className="min-h-screen flex items-center justify-center bg-gray-100  px-4">
			<div className="max-w-lg w-full p-6 my-6 bg-white  shadow-md">
				<div className="mb-6 text-center ">
					<h3 className="text-3xl font-bold text-gray-900">Create Zone</h3>
					<p className="text-lg text-gray-600">Add a new Zone to the list</p>
				</div>
