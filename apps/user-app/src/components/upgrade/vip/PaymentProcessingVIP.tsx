		...
		const fetchStatus = async () => {
			try {
				...
				if (data.message === "success") {
					...
					const res = await fetch("/api/free/upgrade/vip", {
						method: "POST",
						body: JSON.stringify({ paymentId: id }),
					});
					if (!res.ok) {
						toast.error("Failed to create upgrade request to VIP");
					}
					const data = await res.json();
					console.log(data);
					// Optional redirect
...
export default PaymentProcessingVIP;
